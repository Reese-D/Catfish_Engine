    ...
    while (enet_host_service(host_, &event, 0) > 0) {
        switch (event.type) {
            ...
            case ENET_EVENT_TYPE_RECEIVE:
                onPacket(
                    reinterpret_cast<const uint8_t *>(event.packet->data),
...
void NetworkManager::sendReliableTo(ENetPeer *peer, const std::vector<uint8_t> &data) {
    sendPacket(peer, data, CHAN_RELIABLE, true);
}

void NetworkManager::sendToServerUnreliable(const std::vector<uint8_t> &data) {
    if (serverPeer_) sendPacket(serverPeer_, data, CHAN_UNRELIABLE, false);
}

void NetworkManager::sendToServerReliable(const std::vector<uint8_t> &data) {
    if (serverPeer_) sendPacket(serverPeer_, data, CHAN_RELIABLE, true);
}
