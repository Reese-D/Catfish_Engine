...
constexpr float MM_SIZE    = 180.0f; // minimap content area in pixels
constexpr float MM_PADDING =  10.0f;

// World XY → minimap pixel offset from the content-area top-left.
// World Y-axis is flipped: large world Y = top of minimap.
...
// Unproject one NDC point to the Z=0 ground plane. Returns nullopt if the ray
// misses the plane (e.g. looking parallel or away from it).
...
void drawMinimap(const FogOfWar &fog, entt::registry &registry, vk::Extent2D extent) {
    ...
    // --- Background (terrain colour for the whole map) ---
    ...
    for (int y = 0; y < dims.y; ++y) {
        for (int x = 0; x < dims.x; ++x) {
            ...
            ImU32 col = (state == FogState::Hidden)
                            ? IM_COL32(0, 0, 0, 255)
                            : IM_COL32(0, 0, 0, 160);

            float px = origin.x + static_cast<float>(x) * cellPixels;
            float py = origin.y + static_cast<float>(dims.y - 1 - y) * cellPixels; // Y-flip
    ...
    for (auto entity : registry.view<Components::Transform, Components::Faction>()) {
        ...
        if (!isPlayer && !fog.isVisible({t.position.x, t.position.y}))
            continue;
    ...
    if (ImGui::IsWindowHovered() && ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
        ...
        glm::vec2 worldXY = fromMinimap(mouse, origin);
