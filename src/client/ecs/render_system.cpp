...
std::vector<VulkanHelpers::DrawCall> collectDrawCalls(entt::registry &registry, const FogOfWar *fog) {
    std::vector<VulkanHelpers::DrawCall> result;

    auto view = registry.view<Components::Transform, Components::RenderMesh>();
    for (auto entity : view) {
        if (isEnemyInFog(registry, entity, fog))
            continue;
...
void appendSelectionRings(entt::registry &registry, std::vector<VulkanHelpers::DrawCall> &draws, const VulkanHelpers::Model &ringModel, const FogOfWar *fog) {
