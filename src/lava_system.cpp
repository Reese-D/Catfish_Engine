...
std::shared_ptr<VulkanHelpers::Model> createLavaTileModel(
    ...
    verts[0].pos[0] = -0.5f; verts[0].pos[1] = -0.5f; verts[0].pos[2] = 0.0f;
    verts[0].texCoord[0] = 0.0f; verts[0].texCoord[1] = 1.0f;

    verts[1].pos[0] =  0.5f; verts[1].pos[1] = -0.5f; verts[1].pos[2] = 0.0f;
    verts[1].texCoord[0] = 1.0f; verts[1].texCoord[1] = 1.0f;

    verts[2].pos[0] =  0.5f; verts[2].pos[1] =  0.5f; verts[2].pos[2] = 0.0f;
    verts[2].texCoord[0] = 1.0f; verts[2].texCoord[1] = 0.0f;

    verts[3].pos[0] = -0.5f; verts[3].pos[1] =  0.5f; verts[3].pos[2] = 0.0f;
...
void appendLavaDrawCalls(
    ...
    for (float cy = WORLD_MIN; cy < WORLD_MAX; cy += CELL) {
        for (float cx = WORLD_MIN; cx < WORLD_MAX; cx += CELL) {
            ...
            if (!lava.isLava({midX, midY})) continue;
...
void applyLavaDamage(const LavaZone &lava, entt::registry &registry, float dt) {
    ...
    for (auto entity : registry.view<Components::Transform, Components::Health, Components::MovementSpeed>()) {
        ...
        if (!lava.isLava({t.position.x, t.position.y})) continue;
