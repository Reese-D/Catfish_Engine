...
void processAbilityInput(entt::registry &registry, const VulkanHelpers::Window &window, vk::Extent2D extent, std::shared_ptr<VulkanHelpers::Model> projectileModel) {
    ...
    for (auto entity : registry.view<Components::Selected, Components::Ability, Components::Transform, Components::Faction>()) {
        auto &ab = registry.get<Components::Ability>(entity);
        if (ab.timer < ab.cooldown)
            continue;
        ...
        spawnProjectile(registry, projectileModel, t.position, velocity, faction.id, ab.knockbackForce);
        ab.timer = 0.0f;
    }
}
