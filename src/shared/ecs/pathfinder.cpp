...
std::vector<glm::vec3> Pathfinder::findPath(glm::vec2 start, glm::vec2 end) const {
    ...
    constexpr float kCost[8] = {1.41421356f, 1.0f, 1.41421356f, 1.0f, 1.0f, 1.41421356f, 1.0f, 1.41421356f};
    ...
    while (!openSet.empty()) {
        ...
        for (int i = 0; i < 8; ++i) {
            glm::ivec2 nb{cx + kDx[i], cy + kDy[i]};
            if (!isWalkable(nb))
                continue;

            // Don't cut through diagonal gaps
            if (kDx[i] != 0 && kDy[i] != 0) {
                if (!isWalkable({cx + kDx[i], cy}) || !isWalkable({cx, cy + kDy[i]}))
                    continue;
            }

            std::size_t nbIdx = cellIndex(nb);
            float tentativeG = gScore[currIdx] + kCost[i];
